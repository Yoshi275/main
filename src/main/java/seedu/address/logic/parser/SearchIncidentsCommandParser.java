...
public class SearchIncidentsCommandParser implements Parser<SearchIncidentsCommand> {
    ...
    public SearchIncidentsCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argOperatorMap = ArgumentTokenizer.tokenize(args, SEARCH_PREFIX_OPERATOR);
        ...
        if (arePrefixesPresent(argDescMap, SEARCH_PREFIX_DESCRIPTION)) {
            ...
            Name nameKeywords = ParserUtil.parseName(argOperatorMap.getValue(SEARCH_PREFIX_OPERATOR).get());
            return new SearchIncidentsCommand(new OperatorKeywordsPredicate(nameKeywords));
        } else {
